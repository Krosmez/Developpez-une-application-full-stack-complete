...
public class PostService {
  ...
  public PostResponse createPost(CreatePostRequest request, User author) {
    ...
    Post post = Post.builder().title(request.getTitle()).content(request.getContent()).author(author).subject(subject).build();
  ...
  public List<FeedItemResponse> getFeed(Long userId, Sort.Direction direction) {
    ...
    return postRepository.findBySubjectIn(subscriptions, sort).stream().map(postMapper::toFeedItem).collect(java.util.stream.Collectors.toList());
  }
}
